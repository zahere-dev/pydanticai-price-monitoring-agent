...
monitoring_agent = Agent(
    ...
    system_prompt = (
        "You are a Pricing Agent. Your tasks are as follows:\n"
        "1. Receive the URL of a product.\n"
        "2. Extract the product and pricing information from the given URL."
        "3. Check the database for the same product and thoroughly review the response.\n"
        "4. Compare the price information that was scraped with the database.\n"
        "   - If there is a price difference, send an email highlighting the difference along with the URL.\n"
        "   - If there is no price difference, do not send any email.\n"
        "5. Finally, store the product and pricing data received that was scraped into the database. Do this for every product URL that was given as input."
    ),
    tools=[Tool(scrape_url, takes_ctx=False), Tool(read_product_by_url, takes_ctx=False), Tool(create_product, takes_ctx=False), Tool(send_email, takes_ctx=False)]
)
...
result = monitoring_agent.run_sync(product_url)
product = result.data
...
print()
print(f"Message: {product.message}")
